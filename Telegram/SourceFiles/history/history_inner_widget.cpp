...
void HistoryInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	const auto collectForwardItemsForItem = [](
		...
		return HistoryItemsList{ item };
	};
	const auto isAyuForwardForItems = [](const HistoryItemsList &items) {
		return !items.empty()
			&& (AyuForward::isFullAyuForwardNeeded(items.front())
				|| AyuForward::isAyuForwardNeeded(items));
	};
	const auto forwardSelectionItems = collectForwardItemsFromSelected(_selected);
	const auto isAyuForwardSelection = isAyuForwardForItems(
		forwardSelectionItems);
	...
	const auto addItemActions = [&](
		...
		if (Info::Media::MementoForItem(item)) {
			const auto itemId = item->fullId();
			_menu->addAction(
				...
				crl::guard(controller, [=] {
					if (const auto item = session->data().message(itemId)) {
						controller->showSection(
							Info::Media::MementoForItem(item));
					}
				}),
		...
		if (isUponSelected > 1) {
			if (selectedState.count > 0 && selectedState.canForwardCount == selectedState.count) {
				...
				if (ItemsForwardCaptionsCount(forwardSelectionItems) > 0) {
					_menu->addAction(
						tr::ayu_ContextForwardSelectedNoCaption(tr::now),
