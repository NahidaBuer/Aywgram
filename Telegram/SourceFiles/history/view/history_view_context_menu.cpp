...
std::vector<not_null<HistoryItem*>> CollectForwardItemsForItem(
	...
	if (asGroup) {
		if (const auto group = owner->groups().find(item)) {
			return { group->items.begin(), group->items.end() };
		}
	}
	return { item };
}

bool IsAyuForwardForItems(
...
bool HasCaptionsForItems(
		const std::vector<not_null<HistoryItem*>> &items) {
	return ItemsForwardCaptionsCount(
		HistoryItemsList(items.begin(), items.end())) > 0;
}
...
bool AddForwardSelectedAction(
	...
	const auto items = CollectForwardItemsFromSelection(request, list);
...
void AddTopMessageActions(
	...
	if (item
		...
		menu->addAction(
			...
			crl::guard(controller, [=] {
				if (const auto item = controller->session().data().message(
						itemId)) {
