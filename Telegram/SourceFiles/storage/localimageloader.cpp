...
void FileLoadTask::finish() {
	const auto completion = _result
		? _result->to.completion
		: _to.completion;
	...
	const auto fail = [&] {
		if (_album) {
			session->api().failSendingAlbum(_album);
		} else if (completion) {
			completion->fail();
		}
	};
	const auto premium = session->user()->isPremium();
	if (_album && _album->failed) {
		fail();
	} else if (!_result || !_result->filesize || _result->filesize < 0) {
		Ui::show(
			Ui::MakeInformBox(
