...
void Uploader::failed(FullMsgId itemId) {
	...
	if (i != end(_queue)) {
		...
		if (const auto video = _videoWaitingCover.take(*coverId)) {
			...
			if (video->album || video->completion) {
				crl::on_main(this, [=] {
					if (video->album) {
						_api->failSendingAlbum(video->album);
					} else {
						if (const auto item = session().data().message(
								video->fullId)) {
							item->destroy();
						}
						video->completion->fail();
					}
				});
			}
...
void Uploader::notifyFailed(const Entry &entry) {
	const auto completion = entry.file->to.completion;
	const auto album = entry.file->album;
	const auto itemId = entry.itemId;
	if (album || completion) {
		crl::on_main(this, [=] {
			if (album) {
				_api->failSendingAlbum(album);
			} else if (completion) {
				completion->fail();
				if (const auto item = session().data().message(itemId)) {
					item->destroy();
...
void Uploader::finishFront() {
	...
	if (entry.file->type == SendMediaType::Photo) {
		...
		auto ready = UploadedMedia{
			...
			.options = options,
			.completion = entry.file->to.completion,
			.album = entry.file->album,
			.edit = edit,
		};
