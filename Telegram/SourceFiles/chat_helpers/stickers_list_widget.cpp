...
int StickersListWidget::countDesiredHeight(int newWidth) {
	...
	if (_mode == Mode::Full) {
		targetSize = qMax(
			minSize,
			qRound(minSize
				* AyuSettings::getInstance().stickerPanelScale()));
	}
...
bool StickersListWidget::addSearchShortcut(not_null<StickersSet*> set) {
	const auto onlyAdded
		= AyuSettings::getInstance().showOnlyAddedEmojisAndStickers();
	if ((onlyAdded && !SetInMyList(set->flags))
		|| ranges::contains(_searchShortcutSets, set->id, &Set::id)) {
		return false;
	}
