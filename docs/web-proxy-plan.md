# AywGram Desktop WEB proxy: client design

This document records the reviewed client-side design and the implementation present in
this tree. The hosted relay is an independently operated component and is not included
in this repository; it must implement the framing and `MessageChannel` contract described
here. The deployment-dependent verification procedure is intentionally separate in
