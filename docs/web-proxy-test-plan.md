# AywGram Desktop WEB proxy: execution test plan

Execute this document only after an independently operated relay implementing the
protocol in `docs/web-proxy-plan.md` is deployed. Record every requested value and
artifact in one run log. Do not put the proxy secret, Telegram auth keys, message
contents, or the loopback fragment capability in logs or screenshots.
