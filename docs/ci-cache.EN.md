...
The Windows, macOS and Linux workflows share the `ayw-ci-v3` cache namespace. Tag pushes still publish prereleases. Application compiler caching remains enabled for Linux x86_64 and Windows x64. It is disabled for macOS ARM64 by default and unsupported on Windows ARM64 because those compiler wrappers did not complete a clean CI build. Dependency caches remain enabled on every platform.
...
The reusable platform workflows accept `warm_cache=false`, a platform-specific `compiler_cache` default, and optional `source_sha` through `workflow_call`. Compiler caching defaults to false on macOS, defaults to true on Linux and Windows x64, and is ignored on Windows ARM64. Warming requires the default branch, empty `release_tag` and `source_sha`, and both `distribute` and `update_metadata` disabled. Direct platform dispatches support warming and the compiler-cache switch where available.
...
| --- | --- | --- |
| Windows ARM64 | Separate ThirdParty, non-Qt Libraries, and Qt archives with their prepare-stage markers | Disabled; the native sccache 0.17.0 server exits before compilation |
...
| macOS ARM64 | Libraries and ThirdParty in one archive | Disabled by default; the Xcode compiler-wrapper path is retained only for explicit diagnostics |
...
Windows CI uses Ninja Multi-Config with the selected Native Tools environment and the existing Release settings. Local Visual Studio builds are unaffected. Windows x64 may use sccache; Windows ARM64 always invokes the compiler directly. MSVC PCH remains enabled: sccache bypasses PCH compilations, so its x64 benefit applies to the remaining compilations. Read the uncacheable counters before interpreting the overall hit rate.

macOS prepares dependencies and builds the application on one runner with one Xcode selection. Xcode remains the generator to preserve asset catalogs and bundle processing. Automatic builds invoke the selected clang/clang++ directly. The experimental `compiler_cache=true` path retains the ccache wrappers for diagnostics, but is not used by prerelease or default manual builds. Neither the whole Xcode build tree nor final application bundles are cached.
