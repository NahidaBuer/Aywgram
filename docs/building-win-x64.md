...
选择一个空目录作为构建根目录（下文称为 `BuildPath`），例如 `D:\TBuild`。仓库、`Libraries` 和 `ThirdParty` 必须位于这个根目录下：
...
- Git for Windows。
- Visual Studio 2026 的“使用 C++ 的桌面开发”工作负载、MSVC 14.44 x86/x64 ATL，以及 Windows 11 SDK 10.0.26100.0。

构建还需要 Telegram API 的 `api_id` 和 `api_hash`，获取方法见 [Telegram API 凭据](api_credentials.md)。

## 初始化 Visual Studio 环境

Visual Studio 2026 默认的 `v145` 工具集不支持 Windows 7。Telegram Desktop 需要显式选择 MSVC 14.44（`v144.4`，基于仍支持 Windows 7 的 `v143`）。在普通命令提示符中运行：
...
构建产物位于 `BuildPath\AywGram\out\Release\AywGram.exe`。不要提交 `out`、`Libraries`、`ThirdParty` 或其中的缓存文件。

## Qt Visual Studio Tools

如需改善 Visual Studio 中的 Qt 调试体验，可以打开 **扩展 > 管理扩展**，搜索并安装 **Qt Visual Studio Tools**。它只提供 IDE 集成，不负责本仓库所需 Qt 的下载和构建。
